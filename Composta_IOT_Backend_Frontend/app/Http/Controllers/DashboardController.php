...
class DashboardController extends Controller
{
        ...
        public function store(Request $request)
    {
        ...
        if (!$user) {
            return response()->json(["error" => "No autorizado. Usuario no válido."], 401);
        }


        $expectedToken = $user->readings_token;
        ...
        if (!hash_equals($expectedToken, $receivedToken)) {
            return response()->json(["error" => "No autorizado. Token no válido."], 401);
        }
    ...
    public function getTypeAlert(Request $request)
    {
        $idUser = $request->input('idUser');
        $receivedToken = $request->input('api_token');

        if (empty($idUser) || empty($receivedToken)) {
            return response()->json(["error" => "No autorizado. Credenciales incompleta."], 401);
        }
        $user = User::find($idUser);
        if (!$user) {
            return response()->json(["error" => "No autorizado. Usuario no válido."], 401);
        ...
        $typeAlert = $user->type_alert;
        
        // LÍNEA AÑADIDA: Retornar la variable en un JSON
        return response()->json(['type_alert' => $typeAlert], 200);
    }

    public function getLatestJsonData()
    {
        $user = Auth::user();
