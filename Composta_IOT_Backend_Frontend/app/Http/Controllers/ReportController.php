...
class ReportController extends Controller
{
    ...
    {
        $user = Auth::user();
        $rango = $request->query('rango', 'ultimos'); 
    ...
    public function descargarReporte(Request $request)
    {
        ...
        $labels = $datosFiltrados->map(function ($dato) {
            return $dato->date . ' ' . $dato->time;
        })->toArray();
        ...
        $lineConfig = [
            ...
            'options' => [
                ...
                'scales' => [
                    'y' => ['min' => 0, 'max' => 100],
                    'yRight' => ['min' => 0, 'max' => 100], // eje derecho simplificado
    ...
    public function exportReadingsPDF(Request $request)
    {
        $idUser = $request->query('idUser');
        $range  = $request->query('range', 'day'); // valores posibles: day, week, month

        // --- 1. Obtener los datos según rango ---
        $query = DB::table('readings')->where('idUser', $idUser);
        if ($range === 'day') {
            $query->whereDate('date', Carbon::today());
        } elseif ($range === 'week') {
            $query->whereDate('date', '>=', Carbon::now()->subDays(7));
        ...
        $datos = $query->orderBy('date')->orderBy('time')->get();

        if ($datos->isEmpty()) {
            // Si la petición viene desde el navegador o Android, devolvemos una respuesta visible
            if ($request->wantsJson() || $request->ajax()) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'No hay lecturas disponibles para el rango seleccionado.'
                ], 200);
            }

            // Si es una descarga directa (DownloadManager)
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('user.report.readings_empty', [
                'mensaje' => 'No se encontraron datos de lecturas para este rango.'
            ]);

            return $pdf->download('reporte_vacio.pdf');
        }
        ...
        foreach ($datos as $d) {
            if ($d->temperature > $umbralTemperatura) {
                if ($inicio === null) $inicio = $d->date . ' ' . $d->time;
                $fin = $d->date . ' ' . $d->time;
            } elseif ($inicio !== null) {
                $rangosTemperatura[] = ['inicio' => $inicio, 'fin' => $fin];
                $inicio = null;
        ...
        if ($inicio !== null) $rangosTemperatura[] = ['inicio' => $inicio, 'fin' => $fin];
        ...
        if ($inicio !== null) $rangosGases[] = ['inicio' => $inicio, 'fin' => $fin];
        ...
        // --- 5. Gráficos con QuickChart ---
        $labels     = $datos->pluck('time')->toArray();
        $tempsAire  = $datos->pluck('temperature')->toArray();
        $humAire    = $datos->pluck('humidity')->toArray();
        $tempsSuelo = $datos->pluck('ds18b20_temp')->toArray();
        $humSuelo   = $datos->pluck('soil_moisture')->toArray();
        ...
        $doughnutChartUrl = 'https://quickchart.io/chart?width=600&height=400&c=' . urlencode(json_encode($doughnutConfig));
        ...
        // --- 7. Retornar descarga ---
        return $pdf->download("reporte_lecturas_{$range}.pdf");
    }


    public function exportReadingsXLSX(Request $request)
    {
        $idUser = $request->query('idUser');
        $range  = $request->query('range', 'day'); // day, week, month

        // --- 1. Obtener datos según rango ---
        ...
        $datos = $query->orderBy('date', 'desc')
            ->orderBy('time', 'desc')
            ->limit(20)
            ->get();

        if ($datos->isEmpty()) {
            return back()->with('error', 'No hay datos disponibles para este rango.');
        }
        ...
        // --- 3. Análisis automático (igual que PDF) ---
        $analisis = [];
        $analisis[] = ($promedioTempAire >= 20 && $promedioTempAire <= 35)
            ? "Temperatura ambiente óptima."
            : (($promedioTempAire < 20) ? "Temperatura baja." : "Temperatura alta, vigilancia requerida.");
        ...
        // --- 5. Crear export con estructura personalizada ---
        $data = [
            'range'             => $range,
            'datos'             => $datos,
            'promedioTempAire'  => $promedioTempAire,
        ...
        // --- 6. Generar mini gráfico de tendencias (QuickChart) ---
        $labels = $datos->pluck('time')->toArray();
        $tempsAire = $datos->pluck('temperature')->map(fn($v) => $v ?? 0)->toArray();
        $humAire = $datos->pluck('humidity')->map(fn($v) => $v ?? 0)->toArray();
        ...
        $chartUrl = 'https://quickchart.io/chart?width=700&height=300&c=' . urlencode(json_encode($chartConfig));

        $data['chartUrl'] = $chartUrl; // ✅ Agregamos el gráfico al dataset


        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\ReadingsStyledExport($data),
            "reporte_lecturas_{$range}.xlsx"
        );
    }
    ...
    public function exportSalesPDF(Request $request)
    {
        ...
        SELECT
            s.date AS fecha,
            c.name AS cliente,
            CONCAT('Bs ', FORMAT(s.total, 2)) AS total_venta,
            GROUP_CONCAT(
                CONCAT(f.title, ' (', p.amount, ' uds) - Bs ', ROUND((p.amount * f.price), 2))
                ORDER BY f.title SEPARATOR ' \n '
            ) AS detalles_productos
        FROM sales s
        JOIN users c ON c.id = s.idClient
        JOIN payment_products p ON p.idSale = s.id
        JOIN fertilizers f ON f.id = p.idFertilizer
        WHERE s.idUser = ? 
          AND s.date >= ? 
          AND s.date < DATE_ADD(?, INTERVAL 1 DAY)
        GROUP BY s.id, s.date, c.name, s.total
        ORDER BY s.date DESC;
    ";

        $rows = DB::select($sql, [$idUser, $from, $to]);
        ...
        return $pdf->download("ventas_detalladas_{$from}_a_{$to}.pdf");
    }
    ...
    public function exportSalesXLSX(Request $request)
    {
        ...
        s.date AS fecha,
        c.name AS cliente,
        ROUND(SUM(p.amount * f.price), 2) AS total_venta,
        GROUP_CONCAT(
            CONCAT(f.type, ' (', p.amount, ' uds) - Bs ', ROUND(p.amount * f.price, 2))
            ORDER BY f.type SEPARATOR ' \\n '
        ) AS detalles_productos
    FROM sales s
    JOIN users c            ON c.id = s.idClient
    JOIN payment_products p ON p.idSale = s.id
    ...
    WHERE s.idUser = ?
      AND s.date  >= ?
      AND s.date  <  DATE_ADD(?, INTERVAL 1 DAY)
    GROUP BY s.id, s.date, c.name
    ORDER BY s.date DESC;
    ";

        $rows = collect(DB::select($sql, [$idUser, $from, $to]));

        return Excel::download(
            new \App\Exports\SalesExport($rows),
            "ventas_detalladas_{$from}_a_{$to}.xlsx"
        );
    }
}
